...
"Usage: %s [--help|--version] hostdir cmd [arguments]",                        /* FTPC_MSG_USAGE     */

"View or modify FTFS permissions\n"
    "%s [--help|--version] hostdir cmd [arguments]\n"
    "hostdir\treal filesystem directory FTFS stored in\n"
    ...
    "\t\t\t\tentry\tlocal FTFS entry path, starting with \'/\'\n"
    ...
    "\tuser and group permissions are formed from FTFS permissions for corresponding user\n"
    "\tother permissions are formed from OTHF FTFS category, if calling process is run by owner or owner group\n"
    ...
    "\tif ONE OF R/W/X FTFS permission is set it means that corresponding POSIX (stat) R/W/X flag is set,\n"
    "\t(un)setting (chmod) POSIX R/W/X flags (un)sets ALL OF R/W/X FTFS permissions\n"
    "File permissions flags:\n"
    "\tFRD\t(R) read file\n"
    "\tFRA\t(R) stat file (read attributes)\n"
    "\tFRP\t(R) read file permissions (FTFS)\n"
    ...
    "\tDRP\t(R) read directory permissions (FTFS)\n"
    ...
    "However root (uid=0) and FTFS directory owner always granted all the operations\n"
    ...
    "\n\nWarning, if running not under root and not under FTFS owner,\n"
